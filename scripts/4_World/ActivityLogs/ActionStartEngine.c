		...
		if (action_data.m_Player && action_data.m_Player.GetIdentity())
		{
			...
			if (vehCommand)
			{
				...
				{					
					...
					m_MCKLogger.LogMCKActivity("Player " + action_data.m_Player.GetIdentity().GetName() + " (" + action_data.m_Player.GetPosition() + " steam64id=" + action_data.m_Player.GetIdentity().GetPlainId() + ") started " + car.GetDisplayName() + " (" + car.m_CarScriptId + ")");
				}					
			}
		}
    }
