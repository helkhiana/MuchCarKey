		...
		if (action_data.m_Player && action_data.m_Player.GetIdentity())
		{
			...
			{								
				...
				{					
					...
					m_MCKLogger.LogMCKActivity("Player " + action_data.m_Player.GetIdentity().GetName() + " (" + action_data.m_Player.GetPosition() + " steam64id=" + action_data.m_Player.GetIdentity().GetPlainId() + ") exited vehicle " + car.GetDisplayName() + " (ID: " + car.m_CarScriptId + ")");
				}					
			}
		}
	}
